...
export function updateSelectionCount() {
  ...
  const currentPageCount = !currentFileId
    ? window.selections.filter(s => s.page === window.appState.currentPage).length
    : window.selections.filter(s => s.fileId === currentFileId && s.page === window.appState.currentPage).length;

  if (window.appState.files && window.appState.files.length > 1) {
    selectionCount.textContent = `Trang ${window.appState.currentPage}: ${currentPageCount} vùng | File hiện tại: ${currentFileTotal} vùng | Tất cả files: ${totalAllFiles} vùng`;
  } else {
    selectionCount.textContent = `Trang ${window.appState.currentPage}: ${currentPageCount} vùng | Tổng: ${currentFileTotal} vùng`;
  }
}
